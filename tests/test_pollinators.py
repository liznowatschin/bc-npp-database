...
def test_generate_vancouver_pollinator_module_from_tracked_usability(tmp_path):
    ...
    assert result.counts == {
        "pollinator_review": 54,
        "pollinator_evidence_gaps": 378,
...
def test_tracked_vancouver_pollinator_module_validates_cleanly():
    ...
    assert result.counts["pollinator_review"] == 54
    assert result.counts["pollinator_evidence_gaps"] == 54 * len(POLLINATOR_FIELDS)
