...
def test_harden_vancouver_evidence_command_json(tmp_path):
    ...
    assert '"score_readiness": 159' in result.stdout
...
def test_validate_vancouver_evidence_command_json():
    ...
    assert result.exit_code == 0
    assert '"hardened_plant_list": 53' in result.stdout
    assert '"diagnostics": []' in result.stdout
...
def test_generate_vancouver_usability_command_json(tmp_path):
    ...
    assert result.exit_code == 0
    assert '"plant_table": 53' in result.stdout
...
def test_validate_vancouver_usability_command_json():
    ...
    assert result.exit_code == 0
    assert '"use_case_views": 94' in result.stdout
    assert '"diagnostics": []' in result.stdout
...
def test_generate_vancouver_pollinator_module_command_json(tmp_path):
    ...
    assert result.exit_code == 0
    assert '"pollinator_review": 53' in result.stdout
    assert '"pollinator_evidence_gaps": 371' in result.stdout
